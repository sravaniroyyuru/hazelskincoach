...
const Index = () => {
    ...
    if (showWelcome) {
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-background flex flex-col items-center justify-center px-8">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="card-warm p-6 max-w-sm w-full space-y-4">
                    <p className="font-serif text-sm leading-relaxed italic text-foreground/80 whitespace-pre-line">
                        {welcomeNote}
                    </p>
                    <motion.button whileTap={{ scale: 0.97 }} onClick={dismissWelcome} className="w-full h-11 rounded-xl bg-primary text-primary-foreground font-semibold text-sm">
                        I\u2019m ready
                    </motion.button>
                </motion.div>
            </motion.div>
